...
def induced_memory(memory,required):
    ...
    return out
def make_aliases(memory): return {n:"ax-"+hashlib.sha256(("glyphin31|"+n).encode()).hexdigest()[:12] for n in names(memory)}
def inject_collision_aliases(alias_map,memory):
    ns=names(memory); collisions={}
    for j in range(0,min(8,len(ns)),2):
        alias="cx-"+hashlib.sha256(("collision31|"+str(j//2)).encode()).hexdigest()[:10]; collisions[alias]=ns[j:j+2]
    return collisions
...
def make_query(q,aliases):
    ...
    raise ValueError(q)
def make_collision_query(q,alias): return f"resolve alias {alias} for {q}"
def resolve_aliases(alias_index,qtext,expected_count):
    hits=[]
...
def evaluate(mem,v,enc,dec,tok,q,i,structural_index,alias_index,alias_map,collision=False):
    ...
    collisions={alias:cands for alias,cands in alias_index.items() if alias.startswith("cx-")}
    if collision and q!="parameter_retrieval":
        ca=sorted(collisions)[i%len(collisions)]; expected=1; expected_anchors=collisions[ca]; qtext=make_collision_query(q,ca)
        resolved,reason,candidate_count=resolve_aliases(alias_index,qtext,expected); anchor_exact=False; false_positive=resolved is not None; selected=rebuilt; answer_exact=False; required=set(); truth=set(); collision_candidate_exact=candidate_count==len(expected_anchors)
    else:
        aliases=query_aliases(q,a,b,c,alias_map); qtext=make_query(q,aliases); expected=len(aliases); resolved,reason,candidate_count=resolve_aliases(alias_index,qtext,expected)
        ...
        anchor_exact=resolved==expected_anchors; false_positive=resolved is not None and not anchor_exact; truth=ground_truth_closure(mem,q,a,b,c); required=index_closure(structural_index,q,a,b,c)
        if resolved is None: selected=rebuilt; answer_exact=False
        else:
            aa=resolved[0] if resolved else None; bb=resolved[1] if q in ("relationship_exists","path_reconstruction","cross_state_comparison","temporal_ordering") else None; cc=resolved[1] if q=="mixed_multi_hop" else resolved[2] if q=="temporal_ordering" else None
    ...
    full=token_count(tok,encoded)+token_count(tok,qtext); selected_tokens=(token_count(tok,enc(selected)) if q!="parameter_retrieval" and not collision else 0)+token_count(tok,qtext)
    return {"variant":v,"query_type":q,"query_index":i,"collision_case":collision,"state_exact":state_ref.exact,"anchor_exact":anchor_exact,"unique_resolution":resolved is not None and reason=="unique","resolution_reason":reason,"index_exact":required==truth,"answer_exact":answer_exact,"false_positive":false_positive,"collision_candidate_exact":collision_candidate_exact,"anchor_candidates":candidate_count,"total_states":n,"required_states":len(required),"retrieved_state_pct":100*len(required)/n,"full_input_tokens":full,"selected_input_tokens":selected_tokens,"tokens_saved":full-selected_tokens,"selected_token_reduction_pct":100*(full-selected_tokens)/full if full else 0,"query_text":qtext}
def summarize(rows):
    primary=[r for r in rows if not r["collision_case"]]; collision=[r for r in rows if r["collision_case"]]; good=[r for r in primary if r["state_exact"] and r["anchor_exact"] and r["index_exact"] and r["answer_exact"]]
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--output",default="glyphin_simulation31_result.json"); args=ap.parse_args(); tok=tiktoken.get_encoding(TOKENIZER); rows=[]; index_tokens={v:[] for v in VARIANTS}; alias_index_tokens={v:[] for v in VARIANTS}
    for seed in SEEDS:
        for size in SIZES:
            ...
            for v,(enc,dec) in VARIANTS.items():
                rebuilt=dec(enc(mem)); structural_index=build_index(rebuilt); alias_index=build_alias_index(alias_map,collisions)
                index_tokens[v].append(token_count(tok,json.dumps(structural_index,sort_keys=True,separators=(",",":")))); alias_index_tokens[v].append(token_count(tok,json.dumps(alias_index,sort_keys=True,separators=(",",":"))))
                for i,q in enumerate(QUERY_TYPES):
                    rows.append({**evaluate(mem,v,enc,dec,tok,q,i,structural_index,alias_index,alias_map,False),"seed":seed,"size":size})
                    if q!="parameter_retrieval": rows.append({**evaluate(mem,v,enc,dec,tok,q,i,structural_index,alias_index,alias_map,True),"seed":seed,"size":size})
    mean_index={v:statistics.mean(x) for v,x in index_tokens.items()}; mean_alias={v:statistics.mean(x) for v,x in alias_index_tokens.items()}; batches=[]
    for v in VARIANTS:
        vr=[r for r in rows if r["variant"]==v and not r["collision_case"]]
        for b in BATCH_SIZES:
            batch=vr[:b]; full=sum(r["full_input_tokens"] for r in batch); selective=sum(r["selected_input_tokens"] for r in batch); alias_total=mean_alias[v]+selective; combined_total=mean_index[v]+mean_alias[v]+selective
