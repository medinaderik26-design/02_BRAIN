...
def summarize(rows):
    ...
    return {"cases":len(rows),"primary_cases":len(primary),"collision_cases":len(collision),"state_exact_cases":sum(r["state_exact"] for r in rows),"primary_unique_resolution_cases":sum(r["unique_resolution"] for r in primary),"collision_ambiguous_cases":sum(1 for r in collision if r["resolution_reason"]=="ambiguous_alias"),"collision_false_positive_cases":sum(r["false_positive"] for r in collision),"good_cases":len(good),"all_good_cases":len(all_good)}
...
def main():
    ...
    for v in VARIANTS:
        ...
        for b in BATCH_SIZES:
            ...
            batches.append({"variant":v,"batch_size":b,"alias_index_tokens_one_time":mean_alias[v],"full_tokens":full,"indexed_selective_tokens":indexed,"tokens_saved_after_alias_index":full-indexed})
    summary=summarize(rows)
    out={"simulation":31,"benchmark_version":VERSION,"purpose":"Deterministic non-canonical alias/descriptor anchor resolution followed by persistent alias-index dependency retrieval; canonical state names stripped; aliases are opaque tokens not semantic models.","summary":summary,"results":rows,"mean_index_tokens_by_variant":mean_index,"mean_alias_index_tokens_by_variant":mean_alias,"batch_amortization":batches}
