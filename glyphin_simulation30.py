...
def make_query(q,a,b,c):
    # State names are deliberately the only anchor-bearing tokens. This is a
    # deterministic query representation, not natural-language understanding.
...
def resolve_anchors(index,qtext,q,expected_count):
    # Exact lexical resolution against the persistent structural index. Query
    # interpretation is intentionally outside scope: this resolver only finds
    # indexed state IDs literally present in the query string.
...
def evaluate(mem,v,enc,dec,tok,q,i,index):
    ...
    anchor_exact=anchors is not None and anchors==([a,b,c] if expected==3 else [a,b] if expected==2 and q!="mixed_multi_hop" else [a,c] if q=="mixed_multi_hop" else [a] if expected==1 else [])
    ...
    if anchors is None: selected=rebuilt; answer_exact=False
        ...
        aa=anchors[0] if anchors else None; bb=anchors[1] if len(anchors)>1 else None; cc=anchors[2] if len(anchors)>2 else None
