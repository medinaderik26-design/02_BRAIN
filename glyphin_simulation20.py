...
def encode_columnar(memory: GlyphinMemory) -> str:
    ordered = sorted(memory.states); index = {name: i for i, name in enumerate(ordered)}
    ...
    if any(t.utcoffset() is None for t in timestamps): raise ValueError("timestamps must be timezone-aware")
    ...
    base_us = min(utc_us); deltas = [u - base_us for u in utc_us]
    return ";".join([f"{memory.decay_lambda!r},{memory.alpha!r},{memory.beta!r}", names, parents, levels, cohesions, freqs, resonances, sigmas, f"{base_us}:{','.join(map(str, offsets))}", ",".join(map(str, deltas))])

def decode_columnar(text: str) -> GlyphinMemory:
    r = split_escaped(text, ";", True)
    if len(r) != 10: raise ValueError("columnar record must contain ten sections")
    p = split_escaped(r[0], ",")
    ...
    base_s, offsets_s = r[8].split(":", 1); base_us = int(base_s); offsets = [int(x) for x in offsets_s.split(",")]
    ...
    while pending:
        ...
        for i in sorted(pending):
            ...
            if parent_idx and (int(parent_idx) in pending): continue
            ...
            memory.add_state(names[i], level=levels[i], cohesion=cohesions[i], parent=parent, frequency=freqs[i], resonance=resonances[i], sigma=sigmas[i], created_at=created_at)
...
def evaluate(name, enc, dec, memory, tokenizer):
    encoded = enc(memory); reconstructed = dec(encoded); verdict = referee_memory(memory, reconstructed); source = memory.to_json(); m = measure(source, encoded); st = len(tokenizer.encode(source, disallowed_special=())); et = len(tokenizer.encode(encoded, disallowed_special=()))
    return {"variant": name, "states": len(memory.states), "source_chars": m.source_chars, "encoded_chars": m.encoded_chars, "char_reduction_pct": m.char_reduction_pct, "source_tokens": st, "encoded_tokens": et, "token_reduction_pct": (1 - et / st) * 100.0, "exact": verdict.exact, "missing_states": verdict.missing_states, "extra_states": verdict.extra_states, "field_mismatches": verdict.field_mismatches, "parameter_mismatches": verdict.parameter_mismatches}

def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--output", default="glyphin_simulation20_result.json"); args = ap.parse_args(); tokenizer = tiktoken.get_encoding(TOKENIZER_NAME); cases=[]
    for size in SIZES:
        mem = build_memory(size, SEED + size)
        for name,(enc,dec) in VARIANTS.items(): cases.append(evaluate(name,enc,dec,mem,tokenizer))
    summary={}
    for name in VARIANTS:
        rows=[r for r in cases if r["variant"]==name]; reds=[r["token_reduction_pct"] for r in rows]; summary[name]={"mean_token_reduction_pct":sum(reds)/len(reds),"median_token_reduction_pct":statistics.median(reds),"exact_cases":sum(r["exact"] for r in rows),"total_cases":len(rows)}
    ...
    from pathlib import Path; Path(args.output).write_text(json.dumps(out,indent=2,sort_keys=True),encoding="utf-8"); print(json.dumps(summary,indent=2,sort_keys=True))
